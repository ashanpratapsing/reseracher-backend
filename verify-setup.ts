...
const checks = [
  ...
  },
  {
    ...
    check: () =>
      fs.existsSync("src/routes/papers.route.ts") &&
...
if (fs.existsSync(".env")) {
  ...
  const hasValidUrl =
    envContent.includes("https://") && !envContent.includes("your_supabase_url");
  const hasValidKey =
    envContent.includes("SUPABASE_SERVICE_KEY=") &&
    !envContent.includes("your_supabase_service_key");

  console.log(
    `${hasValidUrl ? "✅" : "❌"} SUPABASE_URL: ${hasValidUrl ? "Valid URL" : "Needs valid HTTPS URL"}`
  );
  console.log(
    `${hasValidKey ? "✅" : "❌"} SUPABASE_SERVICE_KEY: ${hasValidKey ? "Present" : "Missing/Placeholder"}`
  );

  if (!hasValidUrl || !hasValidKey) {
    console.log("\n⚠️  Action Required:");
    console.log(
      '   1. Go to https://supabase.com and create a project (if you don\'t have one)'
    );
    console.log(
    ...
    console.log(
      '   4. Copy "Service Role Secret" → Update SUPABASE_SERVICE_KEY in .env'
    );
